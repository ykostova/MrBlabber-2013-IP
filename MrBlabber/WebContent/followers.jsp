	...
	<div class="container">
		...
		<div class="tabbable span8">
			...
			<ul class="nav nav-tabs">
				...
				<li class="active"><a href="./Followers" class="active">Followers</a></li>
					...
					<div class="well span6">
						<ul class="unstyled">
							<li class="clearfix"><p class="text-center">
									<strong>Friends Who Follow You</strong>
								</p>
								<hr class="line-divider" /></li>
							<%
								LinkedList<PersonStore> followers = friendship.getPeopleByFollowee(
										DBconnection.getConnectionInstance(), param);
								if (followers.size() != 0) {
									System.out.println("FOLLOWERS LOADED");
									for (int i = 0; i < followers.size(); i++) {
										String date = String.valueOf(new Timestamp((followers.get(i)).getDate()));
										date = date.substring(0, date.indexOf(" "));
							%>
							<li class="clearfix">
							<label class="float-left controls"><strong><%=followers.get(i).getFirstName()%></strong></label> 
							<label class="float-left controls"><strong><%=followers.get(i).getLastName()%></strong></label>
							<label class="float-left left-margin controls"><small>@<%=followers.get(i).getUsername()%></small></label>
								<label class="left-margin pull-right">Befriended:<%=date%></label> <label
								class="controls span5" name="tweet<%=followers.get(i).getPersonID()%>"><%=followers.get(i).getBio()%></label>
							...
							%>
							<li><label class="controls"> You have NO followers
									yet.</label></li>
							<li class="clearfix"><hr class="line-divider"></li>
							<%
								}
